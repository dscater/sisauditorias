...
const { oTipoTrabajo, limpiarTipoTrabajo } = useTipoTrabajos();
const accion = ref(props.accion_dialog);
const dialog = ref(props.open_dialog);
let form = useForm(oTipoTrabajo.value);
watch(
    () => props.open_dialog,
    (newValue) => {
        dialog.value = newValue;
        if (dialog.value) {
            form = useForm(oTipoTrabajo.value);
        }
    }
);
watch(
...
const enviarFormulario = () => {
    let url =
        form["_method"] == "POST"
            ? route("tipo_trabajos.store")
            : route("tipo_trabajos.update", form.id);
